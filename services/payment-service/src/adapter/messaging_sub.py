...
class PaymentMessagingSubscriber:
    ...
    async def _handle_order_created(self, event_data: dict) -> None:
        ...
        payment_method = event_data.get("payment_method", "AUTOMATIC")
        event_id = event_data.get("metadata", {}).get("event_id", f"order-created-fallback-{order_id}")

        logger.info(f"Received OrderCreated event for materialization (ID: {event_id}): Order={order_id}, Price={total_price}, Qty={quantity}, Store={store_id}, Method={payment_method}")
        ...
        async with db._session_maker() as session:
            try:
                ...
                logger.info(f"CQRS Materialized State: Persisting order locally in payment-service for Order={order_id}: Price={total_price}, Qty={quantity}, Store={store_id}, Method={payment_method}")
