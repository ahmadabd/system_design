...
def do_run_migrations(connection):
    ...
    # Read target_schema set by TenantProvisioner (defaults to "public" if running manually)
    target_schema = config.get_main_option("target_schema", None)

    if target_schema:
        # Activate the tenant schema — all CREATE TABLE will go here
        connection.execute(text(f"SET search_path TO {target_schema}, public"))
        connection.execute(text("COMMIT"))  # SET needs its own transaction in some drivers

    context.configure(
        connection=connection,
        target_metadata=target_metadata,
        version_table_schema=target_schema or "public",  # alembic_version per schema
    )
